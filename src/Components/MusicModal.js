...
const MusicModal = ({ showModal, onClose, onSubmit, fetchMusics }) => {
  ...
  const fetchArtistsAndAlbums = async () => {
    try {
      ...
      const albumsResponse = await axios.get('http://localhost:5432/albums');
  ...
  const onSubmitHandler = async (data) => {
    try {
      ...
      const response = await axios.post('http://localhost:5432/music/addmusic', formData, {
        headers: {
      ...
      if (response.status >= 200 && response.status < 300) {
        ...
        console.error('Error adding music. Status:', response.status);
        toast.error('Error adding music. Status:', response.status);
      }
    } catch (error) {
